...
let rec penultimate_element a_list = 
  match a_list with
  |  [x] -> failwith "Error"
  |  [a; b] -> a
  |  y :: some_other_list -> penultimate_element some_other_list
...
|  _, k when k < 0 -> failwith "Error"
|  [], _ -> failwith "Error"
|  x :: _, 0 -> x 
|  x :: xs, k when k > 0 -> get (xs, k-1)
...
| [x] -> x :: [x]
| a :: remainder -> a :: [a] @ double remainder   
...
let slice i k list =
  let (slice1,_) = divide k list in
  let (_, slice2) = divide i slice1 in
  slice2
...
let rec insert x k = function 
  | [] -> x :: [] 
  | x' :: xs -> if k <= 0 then x :: x' :: xs 
  else x' :: insert x (k-1) xs  
...
let rec rotate n list = 
  let (first, second) = divide n list 
  in second @ first  
